...
class Blank_Inventory_Manager {
    ...
    public function handle_clear_logs() {
        // Check if we're on the right page and the clear logs button was clicked
        if (isset($_POST['clear_logs']) && 
            isset($_GET['page']) && 
            $_GET['page'] === 'blank-inventory-logs' &&
            (!isset($_GET['tab']) || $_GET['tab'] === 'logs')) {
            
            // Verify nonce for security
            if (!isset($_POST['blank_inventory_clear_logs_nonce']) || 
                !wp_verify_nonce($_POST['blank_inventory_clear_logs_nonce'], 'clear_logs')) {
                return;
            }
            ...
            // Clear the logs
            delete_option('blank_inventory_logs');
            
            // Add admin notice
            add_action('admin_notices', function() {
                echo '<div class="updated notice is-dismissible"><p>' . 
                     __('All stock change logs have been cleared.', 'blank-inventory-manager') . 
                     '</p></div>';
            });
            
            // Redirect to prevent form resubmission
            wp_redirect(admin_url('admin.php?page=blank-inventory-logs&tab=logs&logs_cleared=1'));
            exit;
        }
    }
    ...
    private function render_logs_tab() {
        ...
        <?php if (isset($_GET['logs_cleared']) && $_GET['logs_cleared'] === '1'): ?>
            <div class="updated notice is-dismissible">
                <p><?php _e('All stock change logs have been cleared.', 'blank-inventory-manager'); ?></p>
            </div>
        <?php endif; ?>
        
        <?php if (!empty($logs)): ?>
            <form method="post" style="margin-bottom: 10px;">
                <?php wp_nonce_field('clear_logs', 'blank_inventory_clear_logs_nonce'); ?>
                <input type="submit" name="clear_logs" class="button" value="<?php _e('Clear All Stock Change Logs', 'blank-inventory-manager'); ?>" onclick="return confirm('<?php _e('Are you sure you want to clear all stock change logs? This action cannot be undone.', 'blank-inventory-manager'); ?>');">
            </form>
        <?php endif; ?>
